...
printRunEnv () {
  ...
  if [ "$CARDANO_BLOCK_PRODUCER" = true ]; then
    ...
    if [ -v $CARDANO_SHELLY_KES_KEY ]; then
      CARDANO_SHELLY_KES_KEY="$CARDANO_CONFIG_BASE/keys/kes.skey"
    fi
    
    if [ -v $CARDANO_SHELLY_VRF_KEY ]; then
      CARDANO_SHELLY_VRF_KEY="$CARDANO_CONFIG_BASE/keys/vrf.skey"
    fi
    
    if [ -v $CARDANO_SHELLY_OPERATIONAL_CERTIFICATE ]; then
      CARDANO_SHELLY_OPERATIONAL_CERTIFICATE="$CARDANO_CONFIG_BASE/keys/node.cert"
    fi
